...
public class PlayerBuilder : MonoBehaviour
{
...
#if UNITY_EDITOR
        if (_placedLayer < 0)
            Debug.LogWarning($"[PlayerBuilder] �Ҳ���ͼ�� \"{placedLayerName}\"����ʹ��Prefabԭͼ�㡣������ Project Settings �� Tags and Layers �½��ò㣬�����������ɫ�� Ground Layers��");
#endif
    }
    ...
    bool ComputePoseAndCheck(BuildCatalogSO.Entry e, out Vector3 pos, out Quaternion rot)
    {
        ...
        if (e.snapMode == BuildSnapMode.Surface)
        {
            ...
            if (upDot < minUpDotForSurface)    // ����ǽ��/б��������
            {
        ...
        switch (e.snapMode)
        {
            ...
            case BuildSnapMode.Wall:
                ...
                targetPos = hit.point + hit.normal * 0.01f; // ��ǽ
                break;
        }
    ...
    bool AnyBlocking(Collider[] arr, int count, Transform supportRoot)
    {
        for (int i = 0; i < count; i++)
        {
            ...
            if (_ghost != null && col.transform.IsChildOf(_ghost.transform)) continue; // ����Ԥ��
            if (IsPlayerCollider(col)) continue;                                       // �������
            if (supportRoot != null && col.transform.IsChildOf(supportRoot)) continue; // ���Գ������壨�����Ӵ���
            return true;
        }
        return false;
    }
