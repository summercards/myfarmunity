...
namespace UnityEditor.UI
{
    ...
    static internal class MenuOptions
    {
        ...
        [MenuItem("GameObject/Android App View (Experimental)/Android App View 2D", priority = 99)]
        static public void AddAndroidAppView2D(MenuCommand menuCommand)
        {
            //same as AndroidAppViewFeature.IsSupported
            if (EditorUserBuildSettings.activeBuildTarget != BuildTarget.HMIAndroid)
            {
                EditorUtility.DisplayDialog("Please switch to platform HMIAndroid",
                    "Android AppView only support on platform HMIAndroid", "OK");
                return;
            }

            if (!EditorUserBuildSettings.enableAndroidAppView)
            {
                EditorUtility.DisplayDialog("Enable Android AppView",
                    "Please Enable Android AppView in BuildSettings UI", "OK");
                return;
            }

            GameObject go;
            using (new FactorySwapToEditor())
                go = DefaultControls.CreateAndroidAppView2D(GetStandardResources());
            PlaceUIElementRoot(go, menuCommand);
        }
